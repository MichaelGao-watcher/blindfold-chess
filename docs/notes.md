...
**2026-05-17 晚 — 尝试 CLI 并行（失败）**
- 将第4批（StatsModule + ReplayModule + ExitModule）的完整提示词写入 `prompt.md` 附录
- CLI 读取本地 `prompt.md` 后，成功同时 dispatch 3 个 subAgent 后台并行执行
- **结果：全部失败**
  - ExitModule subAgent：15 分钟超时，代码写完但测试未跑完
  - StatsModule subAgent：15 分钟超时，同上
  - ReplayModule subAgent：卡在 WriteFile  approval 审批，无法自动写入文件
- **根因**：
  1. subAgent 默认超时仅 15 分钟，完整实现一个模块（读文件→写代码→写测试→调试通过）实际需要 20~40 分钟
  2. CLI 的 WriteFile approval 是交互式阻塞，后台 subAgent 无法自动通过

**2026-05-17 深夜 — 改为 IDE 串行（成功）**
- 关闭 CLI，回到 IDE 单窗口，按顺序逐个实现：
  1. ExitModule → 14/14 测试通过 ✅
  2. StatsModule → 37/37 测试通过 ✅
  3. ReplayModule → 23/23 测试通过 ✅
- **关键经验**：
  - subAgent 超时可通过 `timeout` 参数调大（最大 3600 秒 = 1 小时），但 CLI 使用者无法直接改默认配置，需在 prompt 中显式声明
  - 并行开发的前提不仅是模块独立，还要确保每个 subAgent 能在超时内完成
  - IDE 串行虽然慢，但审批不阻塞、调试可追问，可靠性远高于 CLI 并行

**subAgent 超时问题速查**
| 问题 | 答案 |
|------|------|
| 默认超时 | 15 分钟（900 秒） |
| 能否调高 | 能，最大 3600 秒（1 小时），在调用 Agent 时传 `timeout` 参数 |
| 用户能直接改吗 | **不能**，是调用方（Orchestrator）配置，需在 prompt 里要求 |
| 不想等提示词里的 "Press ENTER to continue..." | 按 `Ctrl+C` 中断，或关闭终端窗口，已生成的文件不会丢 |

---

## 🏃 SubAgent 接力开发策略（超时/断点续作的标准做法）

> 核心教训：**不要给一个 Agent 一把梭**，把大模块拆成 2~3 轮小任务，每轮 15 分钟足够，而且有明确交付物。

### 错误做法 vs 正确做法

| 做法 | 示例 | 结果 |
|------|------|------|
| ❌ 一把梭 | "实现 StatsModule" | 15 分钟写不完，超时后代码和测试都没完成，全部作废 |
| ✅ 接力式 | "前一个 Agent 已写了框架，你补完剩余功能" | 每轮有明确交付物，超时也不怕，下轮接着干 |
...
**✅ 正确**：
```
你是接力 subAgent。前一个 Agent 已创建 js/stats.js，
实现了 init/getGameHistory/getWinRate 接口，但 getProgressData 和成就徽章未完成，测试也未写。

请：
1. 先读取 js/stats.js 了解现状
2. 补完缺失的 getProgressData() 和 getBadges()
3. 创建 docs/tests/test-stats-node.js 并调通所有测试
4. 只修改你负责的部分，不要动已有代码
```
...
│ 轮次       │ 任务           │ 产出物                        │
├────────────┼────────────────┼───────────────────────────────┤
│ 第1轮      │ 写核心代码框架 │ js/{module}.js（接口+基础功能）│
│ 第2轮      │ 写测试+调通    │ docs/tests/test-{module}-node.js│
│ 第3轮      │ 补UI+集成      │ index.html / css 修改         │
└────────────┴────────────────┴───────────────────────────────┘
```
