...
### 接力开发策略（应对 SubAgent 超时）

如果 subAgent 在 15 分钟内无法完成整个模块（常见），采用**接力模式**：

**拆分模板（每轮一个交付物，15 分钟内完成）**：

| 轮次 | 任务 | 产出物 |
|------|------|--------|
| 第1轮 | 核心代码框架 | `js/{module}.js`（接口定义 + 基础功能） |
| 第2轮 | 补完剩余功能 | 同一文件，补充未完成的函数 |
| 第3轮 | 写测试 + 调通 | `docs/tests/test-{module}-node.js` |
| 第4轮 | UI 集成 | `index.html` / `css/style.css` 修改 |

**接力 prompt 必须包含**：
1. **前人遗产说明**："前一个 Agent 已完成 X，剩余 Y 未做"
2. **读取指令**："先读取 js/{module}.js 了解现状"
3. **边界约束**："只补完缺失部分，不要重写已有代码"
4. **明确交付物**：本轮只负责一个具体任务

> ⚠️ **禁止**：给新 Agent 模糊的"实现这个模块"指令，导致它从零重写。
...
2. 每个接口至少有一个基础实现（可留空或 return mock）
3. 不要写测试，不要改 index.html
...
**场景 B：接力补完（模块文件已存在）**
```
你是接力 subAgent。前一个 Agent 已创建 js/{module}.js，
实现了 [已完成的接口列表]，但 [未完成的接口列表] 尚未实现，测试也未写。

## 你的任务
1. 先读取 js/{module}.js 了解现状
2. 补完缺失的 [具体接口名]
3. 创建 docs/tests/test-{module}-node.js，调通所有测试
4. 只修改你负责的部分，不要推翻已有代码
